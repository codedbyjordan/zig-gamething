...
pub fn build(b: *std.Build) !void {
    ...
    if (target.getOsTag() == .emscripten) {
        const exe_lib = rl.compileForEmscripten(b, "zig-gamething", "src/main.zig", target, optimize);
        ...
        const run_option = b.step("run", "Run zig-gamething");
        run_option.dependOn(&run_step.step);
        return;
    }

    const exe = b.addExecutable(.{ .name = "zig-gamething", .root_source_file = .{ .path = "src/main.zig" }, .optimize = optimize, .target = target });
    ...
    const run_step = b.step("run", "Run zig-gamething");
